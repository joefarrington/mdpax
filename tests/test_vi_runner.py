...
def test_vi_runner_matches_original():
    """Test that ValueIterationRunner matches the original pymdptoolbox implementation."""
    # Parameters
    ...
    vi_orig = mdptoolbox.mdp.ValueIteration(P_orig, R_orig, discount=gamma, epsilon=epsilon)
    vi_orig.run()
    ...
    np.testing.assert_array_equal(
        vi_orig.policy,
        vi_runner.policy,
        err_msg="Policies don't match"
    )

@pytest.mark.parametrize("params", [
    {
        "S": 3, "r1": 4.0, "r2": 2.0, "p": 0.1,
        "gamma": 0.95, "epsilon": 0.001
    },
    {
...
def test_vi_runner_different_parameters(params):
    ...
    vi_orig = mdptoolbox.mdp.ValueIteration(
        P_orig, R_orig,
        discount=vi_params['gamma'],
        epsilon=vi_params['epsilon']
    )
    vi_orig.run()
