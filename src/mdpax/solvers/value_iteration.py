...
class ValueIterationRunner:
    def __init__(
        ...
        Args:
            ...
            output_directory: If None, will create a new directory
            checkpoint_frequency: Frequency with which to save checkpoints, 0 for none
            resume_from_checkpoint: If False, start from scratch;
                if filename, resume from checkpoint
    ...
    def _setup_before_states_actions_random_events_created(self) -> None:
    ...
    def _setup_after_states_actions_random_events_created(self) -> None:
    ...
    def run_value_iteration(
        ...
        Returns:
            A dictionary containing information to log, the final value function
            and, optionally, the policy
        ...
        for i in range(self.iteration, max_iter + 1):
            padded_batched_V = self.calculate_updated_value_scan_state_batches_pmap(
                (self.actions, self.random_event_space, self.V_old),
    ...
    def get_policy(self, V) -> pd.DataFrame:
        ...
        best_action_idxs_padded = self._extract_policy_scan_state_batches_pmap(
            (self.actions, self.random_event_space, V),
            self.padded_batched_states,
        )
    ...
    def _setup(self) -> None:
        ...
        self._setup_before_states_actions_random_events_created()

        # Get the states as tuples initially so they can be used to get
        # state_to_idx_mapping before being converted to a jax.numpy array
        self.states = self.problem.state_space
        ...
        # Reshape states to (N_devices x N_batches x max_batch_size x state_size)
        self.padded_batched_states, self.n_pad = self._pad_and_batch_states_for_pmap(
            self.states, self.batch_size, self.n_devices
        )

        # Get the possible actions
        self.actions = self.problem.action_space
        self.action_labels = self.problem.action_components()

        # Generate the possible random outcomes
        self.random_event_space = self.problem.random_event_space

        # Hook for custom setup in subclasses
        self._setup_after_states_actions_random_events_created()

        if not self.resume_from_checkpoint:
            # Initialise the value function
            self.V_old = self.problem.initial_values()
            self.iteration = 1  # start at iteration 1
        else:
            # Allow basic loading of checkpoint for resumption
            log.info(
                f"Loading initial values from checkpoint: {self.resume_from_checkpoint}"
            )
        ...
        self.output_info["set_sizes"]["N_random_events"] = len(self.random_event_space)

        # Log some basic information about the problem
        log.info("Setup complete")
        log.warning(
            "Changes to properties of the class after setup will not necessarily be \
                reflected in the output and may lead to errors. To run an experiment \
        ...
        log.info(
            f"N random events = {self.output_info['set_sizes']['N_random_events']}"
        )

    def _pad_and_batch_states_for_pmap(
        self, states: chex.Array, batch_size: int, n_devices: int
    ) -> Tuple[chex.Array, int]:
        """Pad states and reshape to
        (N_devices x N_batches x max_batch_size x state_size)
        to support pmap over devices, and using jax.lax.scan to
    ...
    def _calculate_updated_state_action_value(
        ...
        (
            next_states,
            single_step_rewards,
        ) = self._transition_vmap_random_events(
            state,
            action,
            random_event_space,
        )
    ...
    def _calculate_updated_value(
        ...
        """Update the value for a given state, by taking the max of the updated
        state-action values over all actions"""
        return jnp.max(
            self._calculate_updated_state_action_value_vmap_actions(
                state, actions, random_event_space, V_old
            )
        )
    ...
    def _extract_policy_one_state(
        ...
        best_action_idx = jnp.argmax(
            self._calculate_updated_state_action_value_vmap_actions(
                state, actions, random_event_space, V
            )
        )
    ...
    def _extract_policy_scan_state_batches(
        ...
        to loop over batches of states."""
    ...
    def check_converged(
        ...
        Stops when there is approximately no change between estimates of the value
        function at successive iterations"""
        ...
        if max_delta < self.epsilon:
            if iteration >= min_iter:
                ...
                log.info(
                    f"Max delta below epsilon on iteration {iteration}, but min \
                        iterations not reached"
                )
                return False
        else:
