...
class DeMoorPerishable(Problem):
    ...
    def __init__(self,
        ...
        self.state_to_idx_mapping = None
        self.state_component_idx_dict = self.get_state_names()
        self.n_states = None
        self.outcome_to_idx_mapping = self._create_outcome_to_idx_mapping()
    ...
    def get_states(self) -> jnp.ndarray:
        ...
        self.state_to_idx_mapping = jnp.array(state_to_idx, dtype=jnp.int32)
    ...
    def get_state_index(self, state: tuple) -> int:
        ...
        if self.state_to_idx_mapping is None:
            _ = self.get_states()
        return self.state_to_idx_mapping[state]
    ...
    def get_random_outcomes(self) -> jnp.ndarray:
        """Return array of outcomes, demand between 0 and max_demand.
        """
        return jnp.arange(0, self.max_demand + 1).reshape(-1, 1)
    
    def get_outcome_index(self, outcome: str) -> int:
        """Get index in outcome array for a given outcome component"""
        return self.outcome_to_idx_mapping[outcome]
    ...
    def deterministic_transition(
        ...
        demand = outcome[self.outcome_to_idx_mapping["demand"]]
    ...
    def get_outcome_probabilities(
        ...
        Demand is generated from a gamma distribution with mean demand_gamma_mean and CoV demand_gamma_cov.
        """
    ...
    def _create_outcome_to_idx_mapping(self) -> dict[str, int]:
        """Return a dictionary that maps from part of the outcome (specified by a string) to its index"""
        pro_component_idx_dict = {}
        pro_component_idx_dict["demand"] = 0
        return pro_component_idx_dict
    
    ################################################################
    ### Supporting functions for self.deterministic_transition() ###
    ################################################################
