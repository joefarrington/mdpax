...
class MDP:
    ...
    def _compute_transition(
        self, transition: jnp.ndarray | tuple[jnp.ndarray, ...]
    ) -> jnp.ndarray:
...
class ValueIteration(MDP):
    ...
    def _value_iteration_step(
        ...
        done = jnp.logical_or(variation < self.thresh, i >= self.max_iter - 1)
    ...
    def run(self) -> None:
        ...
        if self.verbose:
            ...
            while not (variation < self.thresh or self.iter >= self.max_iter):
                new_V, policy, variation, _done = self._value_iteration_step(
                    V, self.iter
                )
            ...
            def body_fun(
                ...
                new_V, policy, variation, done = self._value_iteration_step(
                    V, iter_count
                )
